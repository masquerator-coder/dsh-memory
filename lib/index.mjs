...
const Config = z.object({
	...
	retrieval: z.object({
		...
		versions: z.union(["active", "all"]).default("active"),
		graph: z.object({
...
function buildPolicy(config) {
	...
	const forgetting = research ? {
		semantic: {
			ttl: config.forgetting?.semantic?.ttl ?? "730d",
			lambda: config.forgetting?.semantic?.lambda ?? 1e-4
		},
		episodic: {
			ttl: config.forgetting?.episodic?.ttl ?? "365d",
			lambda: config.forgetting?.episodic?.lambda ?? .005
		},
		procedural: {
			ttl: config.forgetting?.procedural?.ttl ?? "730d",
			lambda: config.forgetting?.procedural?.lambda ?? .001
		},
		working: {
	...
	const privacy = {
		...
		retrievalFilter: config.privacy?.retrievalFilter ?? (research ? [
			"public",
			"private",
			"confidential"
		] : ["public", "private"]),
	...
	async recall(query) {
		...
		try {
			const result = await withTimeout(recall(policy, this.repo, {
				...
				const statuses = policy.retrieval.versions === "all" ? ["active", "superseded"] : ["active"];
				return (await this.repo.listScope(query.scope)).filter((f) => statuses.includes(f.status) && !isExpired(f, this.now())).map((f) => ({
					fact: f,
					score: 0,
					relevance: 0
			...
			if (policy.retrieval.timeoutMs > 0 && elapsed >= policy.retrieval.timeoutMs) this.metric.incr(MetricKeys.recallTimeout);
			span.finish(true);
			return result;
		} catch (error) {
			this.metric.incr(MetricKeys.recall);
