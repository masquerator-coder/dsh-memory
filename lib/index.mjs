	...
	async recall(query) {
		...
		try {
			const result = await withTimeout(recall(policy, this.repo, {
				...
				const statuses = policy.retrieval.versions === "all" ? ["active", "superseded"] : ["active"];
				return filterByPrivacy((await this.repo.listScope(query.scope)).filter((f) => statuses.includes(f.status) && !isExpired(f, this.now()) && !f.pii), policy.privacy, !policy.privacy.secretRequiresExplicitAuth).slice(0, query.topK ?? policy.retrieval.topK).map((f) => ({
