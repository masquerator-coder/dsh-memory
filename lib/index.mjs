...
const Config = z.object({
	...
	privacy: z.object({
		default: z.union(PRIVACY_LEVELS),
		retrievalFilter: z.array(z.union(PRIVACY_LEVELS)),
...
function resolveProfileKind(profile) {
...
function privacyAllowed(level, filter) {
...
*/
function factAllowed(fact, filter) {
	if (filter.scope !== void 0 && fact.scope !== filter.scope && fact.scope !== "global") return false;
...
*/
function filterByPrivacy(items, policy, allowSecret = false) {
...
var Bm25Index = class Bm25Index {
	...
	add(factId, fact) {
		...
		for (const term of tokenize(factText$1(fact))) terms.set(term, (terms.get(term) ?? 0) + 1);
...
var JsonFileMemoryRepository = class {
	...
	*/
	enqueue(mutation) {
		const run = this.chain.then(async () => {
			...
			try {
				...
				if (previous === void 0) this.applyDelete(id);
	...
	async persist() {
		if (this.filePath === void 0) return;
		if (this.readOnly) throw new Error("memory: refusing to write — the existing memory document could not be read or preserved (see the plugin log)");
	...
	async listScopeIncludingGlobal(scope) {
		await this.chain;
...
var BaseBackend = class {
	...
	async search(_queryText, _queryTerms, _topK) {
		return [];
	}
	async graphNeighbors(_entityId, _relationWhitelist) {
		return [];
	}
	async graphFactIds(_entityId, _topK) {
		return [];
	}
	async remove(factId) {
		this.gate();
...
function features(text) {
	const cjkRe = /[\u3400-\u9fff\u3040-\u30ff\uac00-\ud7af]/g;
	const cjk = text.match(cjkRe) ?? [];
	const units = [...text.replace(cjkRe, " ").toLowerCase().match(/[a-z0-9]+/g) ?? [], ...cjk].filter((s) => s.length > 0);
...
var InMemoryVectorBackend = class extends BaseBackend {
	...
	async upsert(fact) {
		this.gate();
		if (this.items.get(fact.id) !== void 0) this.remDf(fact.id);
	...
	removeInternal(factId) {
		if (this.items.get(factId) === void 0) return;
		this.remDf(factId);
...
var InMemoryGraphBackend = class extends BaseBackend {
	...
	entityToFacts = /* @__PURE__ */ new Map();
	async upsert(fact) {
		this.gate();
		...
		for (const eid of new Set([fact.subject.id, fact.object?.id].filter(Boolean))) {
	...
	removeInternal(factId) {
		...
		for (const eid of /* @__PURE__ */ new Set([edge.from, edge.to])) {
			const set = this.entityToFacts.get(eid);
			set?.delete(factId);
			if (set?.size === 0) this.entityToFacts.delete(eid);
		}
	}
	async graphFactIds(entityId, _topK) {
...
function privacyVisible(privacy, tiers) {
...
*/
function cardVisible(fact, options = {}) {
...
async function buildEntityCard(repo, entityId, options = {}) {
	...
	const visible = facts.filter((f) => cardVisible(f, options)).map((f) => toCardFact(f)).sort((a, b) => b.confidence - a.confidence || b.updated_at - a.updated_at);
	const groups = [];
...
function factPasses(fact, filter) {
	return factAllowed(fact, filter);
}
/** Resolve ids from the derived index into filtered, repo-backed candidates. */
async function resolveHits(repo, hits, filter) {
	const out = [];
	for (const hit of hits) {
		const fact = await repo.get(hit.factId);
		if (fact === void 0 || !factPasses(fact, filter)) continue;
		out.push({
			fact,
...
async function recall(policy, repo, q) {
	...
	const read = q.read;
	let candidates;
	if (read !== void 0 && read.capabilities.search) candidates = await resolveHits(repo, await read.search(q.query, terms, Math.max(topK, policy.retrieval.graph.maxSeedEntities * 3)), filter);
	else candidates = await repo.query(filter, terms, []);
	...
	const graphRead = read !== void 0 && read.capabilities.graph;
	...
	for (let depth = 0; depth < policy.retrieval.graph.maxDepth && frontier.length > 0; depth += 1) {
		...
		for (const entity of frontier) {
			...
			const neighbors = graphRead ? await read.graphNeighbors(entity, whitelist) : await repo.neighbors(entity, whitelist);
	...
	for (const entity of visited) {
		if (graphRead) {
			const resolved = await resolveHits(repo, (await read.graphFactIds(entity, policy.retrieval.graph.maxFanoutPerEntity)).map((fid) => ({
				factId: fid,
				relevance: 0
			})), filter);
	...
	const fused = fusionSort(policy, (policy.retrieval.versions === "all" ? dedupById(all) : dedupBySemanticKey(all)).map((c) => ({
		fact: c.fact,
	...
	return result.map((m) => ({
		...m,
		fact: redactForRecall(m.fact, policy.privacy)
	}));
}
...
function dedupById(candidates) {
	...
	for (const c of candidates) if (!byId.has(c.fact.id)) byId.set(c.fact.id, c);
	return [...byId.values()];
}
//#endregion
...
//#endregion
//#region src/extraction/rules.ts
...
function stripTrigger(text, trigger) {
	...
	if (idx >= 0) rest = rest.slice(idx + trigger.length);
...
*/
function matchRules(text, triggers) {
...
function guessSubject(text, triggerIdx) {
	const before = text.slice(0, triggerIdx).trim();
	if (before.length === 0) return void 0;
	return /([\u4e00-\u9fffA-Za-z]{1,6})\s*$/.exec(before)?.[1] ?? void 0;
}
/**
...
*/
function looksLikeTerminalDump(text) {
	const norm = text.trim();
	if (!norm.includes("\n") && !norm.includes("\r")) return false;
	return /(?:^|\n)\s*(?:PS\s+[^\n>]*>|C:\\[^\n>]*>|[a-zA-Z_-]+@[^\n:]*[>:~$])/m.test(norm) || /Test Files|Tests\s+(?:passed|failed)|\bvitest\b|\bpnpm\s+\w+\b|\bnpm\s+\w+\b|\[\s*\d+\/\d+\s*\]|ELIFECYCLE|ERR_[A-Z_]+|error\s+TS\d+/i.test(norm) && norm.split(/\r?\n/).length >= 3;
}
//#endregion
...
//#endregion
//#region src/application/observability.ts
/**
...
var Metrics = class {
	...
	avgMs(name) {
		...
		if (h === void 0 || h.values.length === 0) return void 0;
	...
	snapshot() {
		...
		for (const [k, h] of this.latencies) if (h.values.length > 0) {
...
var MemoryService = class {
	...
	*  queries vector/graph storage when present). Memoized per policy snapshot. */
	_indexRead;
	get indexRead() {
		if ((this.worker?.backendCount ?? 0) === 0) return void 0;
		if (this._indexRead === void 0) this._indexRead = composeIndexRead(this.worker.backendsSnapshot);
	...
	*/
	async recall(query) {
		...
		try {
			const result = await withTimeout(recall(policy, this.repo, {
				...query,
				requireReadyIndex: this.useOutbox && policy.indexing.requireReadyIndex,
				read: this.indexRead
			}), policy.retrieval.timeoutMs, async () => {
				const statuses = policy.retrieval.versions === "all" ? ["active", "superseded"] : ["active"];
				return filterByPrivacy((await this.repo.listScopeIncludingGlobal(query.scope)).filter((f) => statuses.includes(f.status) && !isExpired(f, this.now()) && !f.pii), policy.privacy, !policy.privacy.secretRequiresExplicitAuth).slice(0, query.topK ?? policy.retrieval.topK).map((f) => ({
	...
	*/
	cardOptions(options = {}) {
		return {
			privacy: this.policy().privacy.retrievalFilter,
			...options
		};
	}
	/**
	...
	*/
	async getCard(entityId, options = {}) {
		const policy = this.policy();
		return withTimeout(buildEntityCard(this.repo, entityId, this.cardOptions(options)), policy.retrieval.timeoutMs, async () => ({
			entityId,
			entityName: entityId,
			entityType: "entity",
			updatedAt: 0,
	...
	*/
	async applyUserMdEdits(scope, markdown) {
		...
		const entityId = await this.primaryUserEntityId(scope);
		const facts = entityId === void 0 ? [] : (await this.repo.byEntity(entityId, { status: ["active"] })).filter((f) => cardVisible(f, options));
	...
	async remember(input) {
		...
		this.trace.start("remember", {
			scope: input.scope,
			factId: outcome.stored.id
		}).finish(true);
		return outcome;
	}
	...
	*/
	extractAndRemember(input) {
		...
		this.queue.enqueue(input.scope, () => this.slowPath({
			...input,
			text
		}));
	...
	async slowPath(input) {
		...
		let assertions;
		if (this.extract !== void 0 && this.llmExtractionEnabled) {
			this.metric.incr(MetricKeys.extractAttempt);
			try {
				assertions = await this.extract(text);
				if (assertions !== void 0 && assertions.length > 0) this.metric.incr(MetricKeys.extractSuccess);
...
function registerMemoryTools(tc) {
	...
	disposers.push(ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (topic !== void 0 && groups.length === 0) groups.push({
				title: topic,
				lines: ["（该主题暂无偏好）"]
			});
			return {
...
function apply(ctx, config) {
	...
	repo.open().then(() => {
		...
		const where = issue.backupPath !== void 0 ? `preserved at ${issue.backupPath}` : "could not be preserved — writes are disabled to avoid data loss";
