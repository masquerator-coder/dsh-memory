...
export class MemoryStore {
    ...
    usage() {
        ...
        for (const e of this.list({ tier: 0, includeArchived: false, includeLowQuality: false })) {
            if (e.layer === 'user')
                user += e.content.length;
            else
                memory += e.content.length;
        }
    ...
    nearCandidates(content, cap = 8) {
        ...
        const rows = this.db.prepare("SELECT * FROM memories WHERE archived = 0 AND content LIKE ? ESCAPE '\\' LIMIT ?").all(like, cap);
        return rows.map(r => this.rowToEntry(r));
    }
    // ---- writes (memories) ---------------------------------------------------
    ...
    recall(query, opts = {}) {
        ...
        try {
            ...
            if (q) {
                for (const row of this.db.prepare('SELECT id FROM memories WHERE rowid IN (SELECT rowid FROM mem_fts WHERE mem_fts MATCH ?)').all(q)) {
    ...
    writeEpisode(id, fields) {
        ...
        try {
            this.upsertEpiStmt.run(id, fields.session_id, fields.ts, fields.summary, fields.tools_used ?? null, fields.topic, fields.extracted ? 1 : 0, fields.archived ? 1 : 0, fields.archived_at ?? null, fields.created);
    ...
    recallEpisodes(query, opts = {}) {
        ...
        try {
            const q = query.split(/\s+/).filter(Boolean).map(t => `"${t.replaceAll('"', '""')}"`).join(' ');
            if (q) {
                for (const row of this.db.prepare('SELECT id FROM episodes WHERE rowid IN (SELECT rowid FROM ep_fts WHERE ep_fts MATCH ?)').all(q)) {
                    ftsIds.add(String(row.id));
                }
            }
        }
