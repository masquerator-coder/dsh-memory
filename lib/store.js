...
export class MemoryStore {
    ...
    identitySyncedIds(target) {
        const rows = this.stmt('SELECT content_id FROM identity_synced WHERE target = ?').all(target);
        return new Set(rows.map(r => r.content_id));
    }
    /** Record that a memory content was written into an identity file (idempotent). */
    markIdentitySynced(content, target, ts = Date.now()) {
        this.stmt('INSERT OR IGNORE INTO identity_synced(content_id, target, ts) VALUES (?,?,?)').run(contentId(content), target, ts);
    }
    identityMetaGet(key) {
        const r = this.stmt('SELECT value AS v FROM identity_meta WHERE key = ?').get(key);
        return r ? Number(r.v) : undefined;
    }
    identityMetaSet(key, value) {
