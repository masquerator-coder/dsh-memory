...
export class MemoryStore {
    ...
    findCanonical(content, layer) {
        ...
        const exact = this.db.prepare('SELECT * FROM memories WHERE content = ? LIMIT 1').get(c);
        if (exact) {
    ...
    getEpisode(id) {
        const r = this.db.prepare('SELECT * FROM episodes WHERE id = ?').get(id);
    ...
    lastEpisodeForSession(sessionId) {
        const r = this.db.prepare('SELECT * FROM episodes WHERE session_id = ? AND archived = 0 ORDER BY ts DESC, rowid DESC LIMIT 1').get(sessionId);
        return r ? this.rowToEpisode(r) : undefined;
    }
    ...
    l2RefinedTs(topic) {
        const r = this.db.prepare('SELECT refined_at AS r FROM l2_refined WHERE topic = ?').get(topic);
    ...
    upsertL2Refined(topic, ts = Date.now()) {
        this.db.prepare('INSERT INTO l2_refined(topic, refined_at) VALUES (?, ?) ON CONFLICT(topic) DO UPDATE SET refined_at = excluded.refined_at').run(topic, ts);
    }
    ...
    identitySyncedIds(target) {
        const rows = this.db.prepare('SELECT content_id FROM identity_synced WHERE target = ?').all(target);
    ...
    identityMetaGet(key) {
        const r = this.db.prepare('SELECT value AS v FROM identity_meta WHERE key = ?').get(key);
