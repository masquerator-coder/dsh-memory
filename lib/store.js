...
export class MemoryStore {
    ...
    applyOne(op, now) {
        ...
        if (op.action === 'remove') {
            ...
            if (op.force) {
                ...
                this.stmt('INSERT INTO forget_deleted(ts, memory_id, content, topic, importance, quality, heat, reason) VALUES (?,?,?,?,?,?,?,?)').run(now, target.id, target.content, target.topic, target.importance, target.quality, heatOf(target, this.forgetDays, now), 'explicit-remove');
    ...
    deleteMemory(id) {
        ...
        this.stmt('INSERT INTO forget_deleted(ts, memory_id, content, topic, importance, quality, heat, reason) VALUES (?,?,?,?,?,?,?,?)').run(now, target.id, target.content, target.topic, target.importance, target.quality, heatOf(target, this.forgetDays, now), 'manual-delete');
        ...
        try {
            ...
            for (const e of this.list({ includeArchived: true, includeLowQuality: true })) {
                ...
                if (shouldDelete(e, observeDays, pendingCorrection(e.content), now)) {
                    ...
                    this.stmt('INSERT INTO forget_deleted(ts, memory_id, content, topic, importance, quality, heat, reason) VALUES (?,?,?,?,?,?,?,?)').run(now, e.id, e.content, e.topic, e.importance, e.quality, heatOf(e, this.forgetDays, now), 'observed-observation-passed');
