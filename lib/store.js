...
export function sqlPathLiteral(path) {
...
export class MemoryStore {
    ...
    stmtCache = new Map();
    constructor(home = resolveDshHome(), budget = DEFAULT_BUDGET, windowDays = 30, forgetDays = resolveForgetDays(), injectThreshold = 1) {
    ...
    static validateBackup(path) {
        ...
        try {
            ...
            // on is missing.
            const colNames = db.prepare('PRAGMA table_info(memories)').all().map((c) => c.name);
            const requiredCols = [
            ...
            const mc = db.prepare('SELECT COUNT(*) AS c FROM memories').get();
            const ec = db.prepare('SELECT COUNT(*) AS c FROM episodes').get();
     ...
     */
    replaceWithBackup(srcPath) {
        const check = MemoryStore.validateBackup(srcPath);
        if (!check.ok)
            throw new Error(`无法导入：${check.error}`);
    ...
    count() {
        const r = this.stmt('SELECT COUNT(*) AS c FROM memories WHERE archived = 0').get();
        return Number(r.c);
    }
    /** Active (non-archived) episode count, without loading rows (P2-25). */
    episodeCount() {
        const r = this.stmt('SELECT COUNT(*) AS c FROM episodes WHERE archived = 0').get();
        return Number(r.c);
    }
    ...
    usage() {
        const rows = this.stmt('SELECT layer AS l, SUM(LENGTH(content)) AS n FROM memories WHERE tier = 0 AND archived = 0 AND low_quality = 0 GROUP BY layer').all();
        let user = 0;
        let memory = 0;
        for (const r of rows) {
            if (r.l === 'user')
    ...
    topicsIndex() {
        const rows = this.stmt('SELECT topic, COUNT(*) AS c FROM memories WHERE tier = 1 AND archived = 0 GROUP BY topic ORDER BY c DESC').all();
    ...
    nearCandidates(content, cap = 8) {
        ...
        if (!slice)
            return [];
        const rows = this.stmt("SELECT * FROM memories WHERE archived = 0 AND content >= ? AND content < ? LIMIT ?").all(slice, `${slice}\uffff`, cap);
        return rows.map(r => this.rowToEntry(r));
    }
    ...
    findCanonical(content, layer) {
        ...
        const exact = this.stmt('SELECT * FROM memories WHERE content = ? LIMIT 1').get(c);
        if (exact) {
        ...
        for (const e of this.list({ includeArchived: false, includeLowQuality: false })) {
            if (layer && e.layer !== layer)
                continue;
            if (contentSimilarity(e.content, c) < SIM_DUP)
                continue;
            if (!best || e.importance > best.importance || (e.importance === best.importance && e.created < best.created))
                best = e;
        }
        return best;
    }
    ...
    nearDuplicates(content, layer) {
        ...
        if (exact) {
            const e = this.rowToEntry(exact);
            if (!e.archived && (!layer || e.layer === layer)) {
                out.push(e);
                seen.add(e.id);
            }
        }
        for (const e of this.list({ includeArchived: false, includeLowQuality: false })) {
            ...
            if (layer && e.layer !== layer)
                continue;
            if (isNearDupCandidate(e.content, c)) {
                out.push(e);
                seen.add(e.id);
            }
        }
    ...
    hardDeleteMemory(id) {
        ...
        if (r.changes > 0 && row)
            this.stmt('DELETE FROM mem_fts WHERE rowid = ?').run(Number(row.r));
        ...
        if (r.changes > 0)
            this.stmt('DELETE FROM failure_memories WHERE memory_id = ?').run(id);
    }
    applyOne(op, now) {
        ...
        if (op.action === 'remove') {
            ...
            if (op.force) {
                ...
                this.stmt('INSERT INTO forget_deleted(ts, memory_id, content, topic, importance, quality, heat, reason) VALUES (?,?,?,?,?,?,?,?)').run(now, target.id, target.content, target.topic, target.importance, target.quality, heatOf(target, this.forgetDays, now, this.windowMs()), 'explicit-remove');
    ...
    enforceBudget(now) {
        ...
        const injectable = (e) => e.layer !== 'user' && isInjectableKind(e.kind) && e.importance >= this.injectThreshold;
        const injectUse = () => totalOf(injectable);
        const usrUse = () => totalOf(e => e.layer === 'user');
        const totalUse = () => totalOf(() => true);
        const demote = (e) => {
    ...
    updateMemory(id, fields) {
        ...
        if (!target)
            return { ok: false, error: `no entry with id ${id}` };
        ...
        if (newContent !== target.content)
            this.recordFailure(target.id, target.content, newContent);
        this.writeMemory(target.id, {
    ...
    deleteMemory(id) {
        ...
        this.stmt('INSERT INTO forget_deleted(ts, memory_id, content, topic, importance, quality, heat, reason) VALUES (?,?,?,?,?,?,?,?)').run(now, target.id, target.content, target.topic, target.importance, target.quality, heatOf(target, this.forgetDays, now, this.windowMs()), 'manual-delete');
     ...
     *  wiped. */
    resetStore() {
        const memCount = this.stmt('SELECT COUNT(*) AS c FROM memories').get();
        const epiCount = this.stmt('SELECT COUNT(*) AS c FROM episodes').get();
    ...
    recall(query, opts = {}) {
        ...
        try {
            ...
            if (q) {
                for (const row of this.stmt('SELECT * FROM memories WHERE rowid IN (SELECT rowid FROM mem_fts WHERE mem_fts MATCH ?)').all(q)) {
        ...
        for (const r of this.stmt(sql).all(...params))
        ...
        for (const e of candidates) {
            ...
            if (hasCJK && !ftsIds.has(e.id) && cjkKw.length > 0) {
                ...
                if (hit === cjkKw.length)
                    base += 6;
                else if (hit >= Math.max(1, Math.ceil(cjkKw.length / 2)))
                    base += 3;
            }
            if (base === 0)
                continue;
    ...
    getEpisode(id) {
        const r = this.stmt('SELECT * FROM episodes WHERE id = ?').get(id);
    ...
    lastEpisodeForSession(sessionId) {
        const r = this.stmt('SELECT * FROM episodes WHERE session_id = ? AND archived = 0 ORDER BY ts DESC, rowid DESC LIMIT 1').get(sessionId);
        return r ? this.rowToEpisode(r) : undefined;
    }
    ...
    replaceEpisodeSummary(id, summary) {
    ...
    recallEpisodes(query, opts = {}) {
        ...
        try {
            const q = query.split(/\s+/).filter(Boolean).map(t => `"${t.replaceAll('"', '""')}"`).join(' ');
            if (q) {
                for (const row of this.stmt('SELECT * FROM episodes WHERE rowid IN (SELECT rowid FROM ep_fts WHERE ep_fts MATCH ?)').all(q)) {
    ...
    hardDeleteEpisode(id) {
        ...
        try {
            ...
            if (r.changes > 0 && row)
                this.stmt('DELETE FROM ep_fts WHERE rowid = ?').run(Number(row.r));
    ...
    listEpisodesForRefine(opts = {}) {
        ...
        const rows = this.stmt(`SELECT * FROM episodes WHERE archived = 0 AND ${status} ORDER BY ts ASC ${limit}`).all();
        return rows.map(r => this.rowToEpisode(r));
    }
    ...
    semanticClusters(opts = {}) {
        ...
        for (const [topic, facts] of byTopic) {
            ...
            if (opts.incremental) {
                ...
                if (refinedAt !== undefined && facts.every(f => f.updated <= refinedAt))
                    continue;
            }
    ...
    writeRefineRun(fields) {
        const r = this.stmt('INSERT INTO refine_runs(ts, level, source_id, prompt_sha, llm_route, decisions, status) VALUES (?,?,?,?,?,?,?)').run(Date.now(), fields.level, fields.sourceId ?? null, fields.promptSha ?? null, fields.route ?? null, fields.decisions, fields.status);
    ...
    refineAttemptCount(sourceId) {
        const r = this.stmt("SELECT COUNT(*) AS c FROM refine_runs WHERE source_id = ? AND status IN ('ok','ok-noop')").get(sourceId);
        return Number(r.c);
    }
    /** M7: last LLM-audit timestamp for a topic cluster (undefined = never audited → audit). */
    l2RefinedTs(topic) {
        const r = this.stmt('SELECT refined_at AS r FROM l2_refined WHERE topic = ?').get(topic);
        return r ? Number(r.r) : undefined;
    }
    /** M7: record that a topic cluster was LLM-audited at `ts` (idempotent upsert). */
    upsertL2Refined(topic, ts = Date.now()) {
        this.stmt('INSERT INTO l2_refined(topic, refined_at) VALUES (?, ?) ON CONFLICT(topic) DO UPDATE SET refined_at = excluded.refined_at').run(topic, ts);
    }
    // ---- correction trail ----------------------------------------------------
    ...
    upsertLessonDraft(memoryId, oldContent, newContent) {
        const now = Date.now();
        try {
            const existing = this.stmt("SELECT id, draft_count FROM lesson_drafts WHERE memory_id = ? AND status = 'draft' ORDER BY id DESC LIMIT 1").get(memoryId);
            if (existing) {
            ...
            const r = this.stmt("INSERT INTO lesson_drafts(memory_id, topic, old_content, new_content, lesson, source, status, draft_count, drafted_at) VALUES (?,?,?,?,?,?,?,?,?)").run(memoryId, 'general', oldContent, newContent, lesson, 'replace', 'draft', 1, now);
    ...
    fireLessonDraft(draftId) {
        if (!this.onLessonDraft)
            return;
        try {
            this.onLessonDraft(draftId);
        }
        catch { /* observer must not break writes */ }
    }
    ...
    listLessonDrafts(opts = {}) {
        ...
        const lim = Number.isFinite(opts.limit) ? Math.max(1, Math.floor(opts.limit)) : 0;
        if (lim > 0)
            sql += ` LIMIT ${lim}`;
    ...
    getLessonDraft(id) {
        const r = this.stmt('SELECT * FROM lesson_drafts WHERE id = ?').get(id);
        if (!r)
            return undefined;
        return {
            id: Number(r.id),
    ...
    markLessonDraftStatus(id, status) {
    ...
    addDraft(input) {
        const now = Date.now();
        try {
            const r = this.stmt('INSERT INTO memory_drafts(session_id, turn, ts, signal, source_text, draft, reason, status, created) VALUES (?,?,?,?,?,?,?,?,?)').run(input.session_id, input.turn ?? null, now, input.signal, input.source_text, input.draft, input.reason ?? null, 'pending', now);
            return Number(r.lastInsertRowid);
        }
        catch {
    ...
    getDraft(id) {
        const r = this.stmt('SELECT * FROM memory_drafts WHERE id = ?').get(id);
        if (!r)
            return undefined;
        return {
            id: Number(r.id),
    ...
    countPendingDrafts() {
        const r = this.stmt("SELECT COUNT(*) AS c FROM memory_drafts WHERE status = 'pending'").get();
        return Number(r?.c ?? 0);
    }
    failureTrail() {
        const rows = this.stmt('SELECT memory_id, old_content, new_content, corrected_at FROM failure_memories').all();
        return rows.map(r => ({
    ...
    pendingCorrectionIn(trail, content) {
    ...
    forgetRun(cfg, now = Date.now()) {
        ...
        try {
            ...
            const pendingCorrection = (content) => this.pendingCorrectionIn(trail, content);
            ...
            for (const e of this.list({ includeArchived: true, includeLowQuality: true })) {
                ...
                if (shouldDelete(e, observeDays, pendingCorrection(e.content), now)) {
                    ...
                    this.stmt('INSERT INTO forget_deleted(ts, memory_id, content, topic, importance, quality, heat, reason) VALUES (?,?,?,?,?,?,?,?)').run(now, e.id, e.content, e.topic, e.importance, e.quality, heatOf(e, this.forgetDays, now, this.windowMs()), 'observed-observation-passed');
            ...
            for (const ep of this.listEpisodes({ includeArchived: true })) {
                ...
                if (now - ep.archived_at > observeDays * DAY_MS) {
                    ...
                    this.stmt('INSERT INTO forget_deleted_episodes(ts, episode_id, session_id, summary, topic, tools_used, reason) VALUES (?,?,?,?,?,?,?)').run(now, ep.id, ep.session_id, ep.summary, ep.topic, ep.tools_used ?? null, 'episode-observation-passed');
