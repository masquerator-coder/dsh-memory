...
export function buildSection(store, opts = {}) {
    ...
    // NOTE (2026-08-31, user.md 人写权威化): the user layer is deliberately NOT
    // injected here. user.md is now the authoritative, human-authored portrait,
    // injected separately as the constant `memory:user` section (M9). Injecting
    // layer=user tier-0 memories here too would double-present the same picture
    // and churn the KV prefix on every assembly. User-layer memories still
    // accumulate in the store and remain recallable via memory_recall.
