...
export function formatEntries(entries) {
    return entries.map(e => `[${e.layer}/${e.tier}${e.low_quality ? '/低质' : ''} i=${e.importance}] (${e.id}) ${e.topic}: ${oneLine(e.content)}`).join('\n');
}
/** Render episodic (session) memories. (P0-6) same one-line guard on summary. */
export function formatEpisodes(episodes) {
    return episodes.map(ep => `[${ep.topic}] ${ep.session_id} · ${oneLine(ep.summary)}${ep.tools_used ? ` (工具:${ep.tools_used})` : ''}`).join('\n');
}
