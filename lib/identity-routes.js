 ...
 *  M5 (audit 2026-09-03): spawn with array args instead of an exec-built shell
 *  string — the old ad-hoc escaping broke on cmd metacharacters (`&`, `%VAR%`)
 *  in the path. On win32 `start` is a cmd builtin, so cmd.exe is still the
 *  carrier, but the path is passed as one manually-quoted argument with
 *  windowsVerbatimArguments (Node's default escaping only quotes spaces, which
 *  still leaves `&` inert-unquoted). */
function openInLocalEditor(absPath) {
    return new Promise((resolve) => {
        let child;
        if (process.platform === 'win32') {
            // `cmd /c start "" "<path>"` launches via the file's default association;
            // strip any embedded quotes so the manual quoting stays well-formed.
            const quoted = `"${absPath.replace(/"/g, '')}"`;
            child = spawn('cmd.exe', ['/c', 'start', '', quoted], { windowsVerbatimArguments: true, stdio: 'ignore' });
        }
        else if (process.platform === 'darwin') {
            child = spawn('open', [absPath], { stdio: 'ignore' });
        }
        else {
            child = spawn('xdg-open', [absPath], { stdio: 'ignore' });
        }
        child.on('error', (err) => resolve({ ok: false, error: err.message }));
        child.on('close', (code) => resolve(code === 0 ? { ok: true } : { ok: false, error: `editor exited with code ${code}` }));
    });
}
    ...
    const memoryEdit = (req, res) => {
        void (async () => {
            try {
                ...
                const result = store.updateMemory(id, {
    ...
    const memoryReset = (req, res) => {
        void (async () => {
            try {
                ...
                const result = { ok: true, memories: r.memories, episodes: r.episodes, backedUp: r.backedUp };
