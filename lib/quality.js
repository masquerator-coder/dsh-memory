...
export function longestCommonSubstr(a, b) {
    const n = a.length;
    const m = b.length;
    if (n === 0 || m === 0)
        return 0;
...
export function tokenize(s) {
    ...
    for (const m of low.matchAll(/[a-z0-9]+/g))
        t.add(m[0]);
...
export function tokenContain(a, b) {
...
export function isNearDupCandidate(a, b) {
    const l = contentSimilarity(a, b);
    if (l >= 0.85)
        return true;
    if (l >= 0.55 && tokenContain(a, b) >= 0.55)
        return true;
    return false;
}
export function qualityScore(content, existing) {
...
export function isLowQuality(score) {
    return score <= LOW_QUALITY;
}
/** Injection weight multiplier (0 = never injected; degraded entries down-weighted). */
export function weightOf(score) {
    if (score >= DEGRADED_HIGH)
        return 1;
    if (score <= LOW_QUALITY)
        return 0;
    return score / 100;
}
