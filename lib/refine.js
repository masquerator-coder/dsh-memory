...
export function manualRefineOverride(mode, provider, model) {
    if (mode !== 'manual')
        return undefined;
    if (provider && model)
...
export function buildL1RetryUser(summary, toolsUsed, badRaw) {
...
export function buildLessonJudgePrompt(drafts) {
    const system = 'You are the lesson judge of an AI memory system. A stored fact was corrected. ' +
...
export function parseLessonJudgements(text) {
    const arr = parseJsonArray(text);
    if (arr === null)
        return null;
    const out = [];
    for (const item of arr) {
        ...
        if (!Number.isInteger(index) || index < 0)
            continue;
        if (it.decision !== 'promote' && it.decision !== 'drop')
            continue;
 ...
 */
export async function runRefineLessonPromote(store, input = {}) {
    const stats = { promoted: 0, dropped: 0, degraded: 0, runIds: [] };
    ...
    if (raw === null || judgements === null) {
        ...
        for (const d of drafts) {
            try {
                stats.runIds.push(store.writeRefineRun({ level: 3, sourceId: String(d.id), route, decisions: '[]', status: 'degraded' }));
            }
            catch { }
        }
    ...
    for (const j of judgements) {
        ...
        try {
            if (j.decision === 'drop') {
                ...
                store.batch([{ action: 'add', layer: 'memory', kind: 'lesson', importance: (j.importance ?? 3), topic: d.topic, content: lessonText }], 'lesson-promote');
                store.markLessonDraftStatus(d.id, 'promoted');
...
export function buildAddMetaPrompt(content) {
    const system = 'You classify one memory fact for an AI memory system. Output ONLY JSON: ' +
...
export function parseAddMetaJson(text) {
 ...
 */
export async function judgeAddMeta(store, content, input) {
    if (!input.llm || !input.provider || !input.model)
        return null;
