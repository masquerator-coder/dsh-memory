...
export const DRAFT_SIGNAL_LABEL = {
...
const AGENT_ROOTCAUSE_PATTERNS = [
...
function isRootcauseTool(name) {
    if (ROOTCAUSE_TOOLS.has(name))
        return true;
...
function extractDraft(source, cap = 160) {
 ...
 */
export function detectDraftSignal(input) {
 ...
 */
export function collectTurnRoleTexts(events, turn) {
    const user = [];
    const agent = [];
...
export function collectTurnToolsPlain(events, turn) {
    const out = [];
    const seen = new Set();
    for (const raw of events) {
        const ev = raw;
 ...
 */
export function runDraftCapture(store, input) {
    try {
