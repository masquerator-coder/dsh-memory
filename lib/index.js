...
import { isSuppressed, manualRefineOverride, resolveRefineRoute, runRefineL1, runRefineL2, runRefineLessonPromote } from './refine.js';
import { runDraftCapture } from './draft.js';
import { autocreateIdentityFiles } from './identity.js';
import { registerControlRoutes } from './identity-routes.js';
...
export function apply(ctx, config = {}) {
    ...
    }, config.windowDays ?? WINDOW_DAYS, undefined, // forgetDays default
    ...
    const manualRefine = () => manualRefineOverride(runtime.refineModelMode, runtime.refineModelProvider, runtime.refineModel);
    ...
    ctx.effect(() => {
        ...
        const controls = {
            ...
            loadModels: async () => ({ default: {}, candidates: [], failures: [] }),
        };
        ...
        if (timeInjection) {
            const timeSource = new TimeSource({
                ...
                onError: (err) => { if (!disposed)
                    console.warn('[dsh-memory] internet time pin failed:', err instanceof Error ? err.message : err); },
            });
            timeSource.start();
        ...
        ctx.systemPrompt.section({
            ...
            text: () => {
                if (!runtime.enabled)
                    return '';
                if (!runtime.customPromptEnabled)
                    return ''; // 2026-09-06: master switch for this section
        ...
        const l0Dispose = ctx.on('session/event', (session, event) => {
            ...
            if (session.id && runtime.draftCaptureEnabled) {
                runDraftCapture(store, {
                    ...
                    sessionId: session.id,
                    onError: (err) => { if (!disposed)
                        console.warn('[dsh-memory] draft capture failed:', err instanceof Error ? err.message : err); },
                });
            }
            ...
            if (session.id) {
                ...
                const newEvents = session.snapshotEvents().slice(cursor);
        ...
        let refining = false;
        const runRefine = async (force = false) => {
            ...
            try {
                ...
                // for it explicitly.
                if (!force && runtime.peakHourSuppress && isSuppressed(new Date(), suppressCfg))
                    return;
                if (l1Enabled) {
                    const l1Route = resolveRefineRoute(manualRefine() ??
                ...
                if (l2Enabled) {
                    const l2Route = resolveRefineRoute(manualRefine() ??
                ...
                if (runtime.lessonDraftEnabled) {
                    const lessonRoute = resolveRefineRoute(manualRefine() ??
                        ((config.l0Provider && config.l0Model)
                            ? { provider: config.l0Provider, model: config.l0Model }
                            : undefined), learned, hostDefault);
        ...
        const lessonInstant = async () => {
            if (disposed || !runtime.enabled)
                return;
            if (!runtime.lessonInstantJudge || !runtime.lessonUseLlm || !runtime.lessonDraftEnabled)
                return;
            const route = resolveRefineRoute(manualRefine() ??
                ((config.l0Provider && config.l0Model)
                    ? { provider: config.l0Provider, model: config.l0Model }
                    : undefined), learned, hostDefault);
            await runRefineLessonPromote(store, {
        ...
        controls.runNow = async () => {
            ...
            const done = (async () => {
                try {
                    ...
                    if (!disposed)
                        console.warn('[dsh-memory] manual refine failed:', err instanceof Error ? err.message : err);
                }
            })();
            track(done);
            await done;
        ...
        controls.loadModels = async () => {
            ...
            try {
                const sel = ctx.agentDefaultModel?.currentSelection?.();
                if (sel?.provider && sel?.model)
                    hostDefaultModel = { provider: sel.provider, model: sel.model };
            }
