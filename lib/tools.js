...
export function registerMemoryTools(ctx, store, opts = {}) {
    ...
    const memoryTool = defineTool({
        name: 'memory',
        description: '持久记忆。action=list 查看;add 记稳定事实(用户偏好/环境/可复用约定),replace 纠正(需 id),remove 删除(需 id,force=true 物理删除否则软归档)。layer=user 记用户事项,memory 记观察。importance 1-5,kind 自动推断。写入即时全局生效。',
        parameters: {
