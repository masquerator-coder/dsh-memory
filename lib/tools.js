...
export function registerMemoryTools(ctx, store, opts = {}) {
    ...
    const memoryTool = defineTool({
        name: 'memory',
        description: '写/查全局持久记忆(跨会话)。action: list 查看;add 新增;replace 纠正(需 id);remove 删除(需 id,force=true 物理删除,否则软归档)。layer: user=用户事项,memory=观察。importance 1-5;kind 自动推断;写入即时全局生效。',
        parameters: {
