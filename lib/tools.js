...
const ACTIONS = ['add', 'replace', 'remove', 'list'];
const SCOPES = ['semantic', 'episodic', 'all'];
/** memory_drafts 动作联合(独立于 ACTIONS,兼容 pick 闭集约束)。 */
const DRAFT_ACTIONS = ['list', 'promote', 'discard'];
...
function summarizeResult(store) {
    const u = store.usage();
    return `当前记忆 ${store.count()} 条,核心占用 ${u.pct}%(${u.total}字符)。`;
}
export function registerMemoryTools(ctx, store, opts = {}) {
    const objectOutput = {
        schema: {
            type: 'object',
    ...
    const callCard = (title, kind, rawInput) => ({
        card: 'generic', title, kind, ...(rawInput ? { rawInput } : {}),
    });
    const resultCard = (title, opts = {}) => ({
    ...
    const memoryTool = defineTool({
        name: 'memory',
        description: '写/查全局持久记忆(跨会话)。action: list 查看;add 新增;replace 纠正(需 id);remove 删除(需 id,force=true 物理删除,否则软归档)。layer: user=用户事项,memory=观察。importance 1-5;kind 自动推断;写入即时全局生效。content 须为最精炼的单句「陈述事实」(如 User prefers concise responses),禁止写出祈使句/指令(如 Always respond concisely)。注意:运行时上下文/系统提示词/会话进度不属于可记录的用户事实,勿 add。',
        parameters: {
        ...
        async execute(args, exec) {
            ...
            if (!action)
                return { content: `[FAIL] 未知 action "${rawAction}";可选 list|add|replace|remove。${summarizeResult(store)}` };
            const op = {
                action,
            ...
            if (res.overflowed)
                return { content: `[FAIL] 未写入: 常驻核心(importance≥5)占用已达上限(${res.usage.pct}%),降级已无法腾出空间——普通(importance<5)记忆本会自动降级保存,不受此拒。当前常驻核心:\n${formatEntries(res.entries.filter(e => e.tier === 0))}\n如需保存该受保护核心: 请用 memory replace 合并相近旧条目,或调低 importance 让其降级保存,再重试。` };
            if (res.rejected.length > 0)
                return { content: `[FAIL] 未完成: ${res.rejected.map(r => r.reason).join('; ')}。${summarizeResult(store)}` };
        ...
        presentCall(args) {
            ...
            if (action === 'list')
                return callCard('查看记忆', 'search', str(args.topic) ?? '');
    ...
    const recallTool = defineTool({
        ...
        async execute(args, _exec) {
            ...
            let topK = 8;
            if (typeof args.topK === 'number' && Number.isFinite(args.topK))
                topK = Math.max(1, Math.min(TOPK_MAX, Math.floor(args.topK))); // P2-33: clamp
    ...
    const userProfileTool = defineTool({
        ...
        description: '读取用户画像(user.md)。画像内容默认不注入系统提示以节省上下文;当回答、决策或个性化确实需要了解用户(身份/偏好/工作环境/习惯)时,调用本工具获取完整画像。无参数。',
        parameters: {},
        output: objectOutput,
        async execute(_args, _exec) {
            ...
            if (!u)
                return { content: '用户画像(user.md)尚未维护或为空;如需建立,可在设置中编辑 user.md,或用 memory(layer=user) 记录用户事实。' };
            return { content: u };
        },
        presentCall() {
...
function registerDraftTools(ctx, store, opts = {}) {
    ...
    const draftTool = defineTool({
        name: 'memory_drafts',
        description: '处理 turn-end 纯规则捕获的"待沉淀技术经验草稿"(零 LLM 兜底,LLM 忙也不会丢)。' +
        ...
        async execute(args, exec) {
            ...
            if (hits.length > 0) {
                return {
                    content: `[提示] 库中已有相似记忆,为避免重复,请先用 memory replace 合并或跳过(草稿 #${id} 仍为 pending):\n` +
                        formatEntries(hits.map((h) => h.entry)),
                };
            }
            const op = {
        ...
        presentCall(args) {
            ...
            if (action === 'list')
                return callCard('待沉淀草稿', 'search');
            if (action === 'discard')
                return callCard('丢弃草稿', 'delete', `#${String(args.id ?? '')}`);
