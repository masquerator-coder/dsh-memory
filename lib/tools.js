...
export function registerMemoryTools(ctx, store, opts = {}) {
    ...
    const memoryTool = defineTool({
        ...
        async execute(args, exec) {
            ...
            if (!action)
                return { content: `未知 action "${rawAction}";可选 list|add|replace|remove。${summarizeResult(store)}` };
            const op = {
                action,
            ...
            if (res.overflowed)
                return { content: `记忆预算已满;本次未写入。当前核心(${res.usage.pct}%):\n${formatEntries(res.entries.filter(e => e.tier === 0))}\n请先用 memory replace/remove 整合后再写。` };
            if (res.rejected.length > 0)
                return { content: `未完成: ${res.rejected.map(r => r.reason).join('; ')}。${summarizeResult(store)}` };
