...
export function buildSection(store: MemoryStore, opts: { importanceThreshold?: number } = {}): SectionBuild {
  ...
  if (coreUser.length > 0) {
    ...
    for (const e of coreUser) rows.push(`- <memory-entry topic="${escHtml(sanitizeText(e.topic, 40))}">${sanitizeText(e.content)}</memory-entry>`)
  }
  if (coreMem.length > 0) {
    rows.push('## memory · 偏好/环境', `占用 ${usage.memory}字符（单条≤${ENTRY_CAP}）。`)
    for (const e of coreMem) rows.push(`- <memory-entry topic="${escHtml(sanitizeText(e.topic, 40))}">${sanitizeText(e.content)}</memory-entry>`)
  }
  if (topics.length > 0) {
