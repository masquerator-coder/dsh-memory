 ...
 */
export const TIME_ENDPOINTS: readonly string[] = [
 ...
 */
export async function fetchInternetEpochMs(
  endpoints: readonly string[] = TIME_ENDPOINTS,
  ...
  for (const url of endpoints) {
    try {
      ...
      const secs = Number(
        body?.unixtime ?? body?.epochSeconds ?? body?.unixtime_ms ?? body?.milliseconds,
      )
...
export class TimeSource {
  private skewMs = 0
  private source: TimeSourceKind = 'local'
  ...
  async refresh(): Promise<boolean> {
    if (this.refreshing || this.disposed) return this.source === 'internet'
 ...
 */
export function formatLocalDate(epochMs: number, timeZone = resolveSystemTimeZone()): string {
  try {
