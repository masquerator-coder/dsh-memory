...
/**
 * Idempotent column-migration extension point. v3.0.0 ships a full DDL, so there
 * is nothing to migrate yet. Future v3.x columns go here as guarded ADD COLUMN;
 * until then this is an honest no-op (not a misleading stub). (P2-22)
 */
export function migrateColumns(_db: DatabaseSync): void {
  /* No-op until the first v3.x schema change lands. */
}
